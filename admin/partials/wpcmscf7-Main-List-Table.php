...
class WPCMSCF7_Main_List_Table extends WP_List_Table
{
    /**
    ...
    {   
        ...
        while ( $the_query->have_posts() ) : $the_query->the_post();
            ...
            $title = get_the_title();
            $enable = array();
            $enable = get_post_meta($form_post_id, 'wpcmscf7_enable');
            $link  = "<a class='row-title' href=admin.php?page=wpcmscf7-list.php&fid=$form_post_id>%s</a>";
            $data_value['name']  = sprintf( $link, $title );
            $data_value['noofentry'] = sprintf( $link, $totalItems );
            if(isset($enable[0]) && !empty($enable[0]))
                $data_value['paypal'] = sprintf( $link, "Yes" );
            else
                $data_value['paypal'] = sprintf( $link, "No" );
            $data[] = $data_value;
        endwhile;
